...
from umbral import (
    ...
    reencrypt, decrypt_reencrypted, Capsule
)
import base64
import json
...
def encrypt_emr():
    ...
    # Encrypt EMR data
    try:
        capsule, ciphertext = encrypt(patient_keys['public_key'], plaintext.encode('utf-8'))
    except Exception as e:
...
def generate_kfrags_endpoint():
    ...
    for doctor_id in doctor_ids:
        ...
        try:
            ...
            return jsonify({'error': f'Failed to generate KFrags for doctor {doctor_id}: {str(e)}'}), 500
...
def decrypt_emr():
    ...
    if not patient_id or not doctor_id or not capsule_str or not ciphertext_str:
        return jsonify({'error': 'Missing patient_id, doctor_id, capsule, or ciphertext'}), 400
